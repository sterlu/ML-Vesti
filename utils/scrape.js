...
const scrapeNews = async (continuation = '') => {
  ...
  fs.appendFileSync('novi_podaci/pred_naslovi.input', parsed.map(i => i.hypeTitle).filter(a => a.length).join('\n'));
  fs.appendFileSync('novi_podaci/post_naslovi.input', parsed.map(i => i.normalizedTitle).filter(a => a.length).join('\n'));
  fs.appendFileSync('novi_podaci/puni_naslovi.input', parsed.map(i => i.title).filter(a => a.length).join('\n'));
